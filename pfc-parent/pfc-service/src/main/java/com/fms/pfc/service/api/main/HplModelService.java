...
public class HplModelService {
	...
	public void save(List<HplModelDto> modelList, Integer parentId, boolean isCreate, Date currentDate, String userId, List<HplModelDto> modelDelList) {
		...
		if (isCreate) {
			...
			for (HplModelDto hmDto : modelList) {
				if (StringUtils.equalsIgnoreCase(hmDto.getIndicator(), "exist")) {
					HplModelDto existing = findDtoById(hmDto.getPkHplModelId());
					hmDto.setCreatorId(existing.getCreatorId());
					hmDto.setCreatedDatetime(existing.getCreatedDatetime());
					hmDto.setModifierId(userId);
					hmDto.setModifiedDatetime(currentDate);

				} else if (StringUtils.equalsIgnoreCase(hmDto.getIndicator(), "new")) {
					hmDto.setFkHplId(parentId);
					hmDto.setCreatorId(userId);
					hmDto.setCreatedDatetime(currentDate);
				}
				hplModelRepo.saveAndFlush(hplModelConv.dtoToEntity(hmDto));
			}

			// For edit mode -> delete from list
			for (HplModelDto hmDto : modelDelList) {
				hplModelRepo.deleteById(hmDto.getPkHplModelId());
			}
