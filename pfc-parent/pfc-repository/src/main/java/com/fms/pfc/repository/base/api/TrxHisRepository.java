...
public interface TrxHisRepository extends JpaRepository<TranxHistory, String> {
	...
	@Query(value = "SELECT "
			...
			+ "CASE WHEN (A.RECORD_TYPE_ID = 201 AND A.FUNCTION_TYPE NOT IN (4,7) AND ISNUMERIC(A.RECORD_REF) = 1) THEN (SELECT RAW_MATL_NAME FROM RAW_MATERIAL WHERE RAW_MATL_ID = A.RECORD_REF) "
			+ "WHEN (A.RECORD_TYPE_ID = 301 AND A.FUNCTION_TYPE NOT IN (4,7) AND ISNUMERIC(A.RECORD_REF) = 1) THEN (SELECT PR_NAME FROM PRODUCT_RECIPE WHERE PR_ID = A.RECORD_REF) "
			+ "WHEN (A.RECORD_TYPE_ID = 11 AND A.FUNCTION_TYPE NOT IN (4,7)) THEN (SELECT COUNTRY_NAME FROM REF_COUNTRY WHERE COUNTRY_ID = A.RECORD_REF) "
			+ "WHEN (A.RECORD_TYPE_ID = 12 AND A.FUNCTION_TYPE NOT IN (4,7)) THEN (SELECT VENDOR_NAME FROM VENDOR WHERE VENDOR_ID = A.RECORD_REF) "
