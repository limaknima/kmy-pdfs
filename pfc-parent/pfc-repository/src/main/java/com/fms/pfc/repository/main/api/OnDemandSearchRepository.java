...
public interface OnDemandSearchRepository extends JpaRepository<OnDemandSearch, String> {
	@Query(value = "select * from ( "
			...
			+ "	case"
			+ "     when (t2.PFILE_ID) is NULL then 'File not found'"
			+ "		when (t2.FILE_NAME = '' OR t2.FILE_NAME is NULL) then 'File name is blank'"
			...
			//+ "	 ) "
			//+ " ) "
			+ "and (?6 = '' OR t2.FILE_PATH in ( "
			+ "	 select t3.REL_ID from REL_PATH2 t3 where t3.REL_ID = t2.FILE_PATH "
			+ "	 and t3.FILE_PATH like case when ?7 = '1' then concat('%', ?6, '%') "
			+ "	 when ?7 = '2' then concat('%', ?6) "
			+ "	 when ?7 = '3' then ?6 "
			+ "	 when ?7 = '4' then concat(?6, '%') end "
			+ ") ) "
			+ "and (ISNUMERIC(t1.YEAR) = 1 and t1.YEAR <= '3000') "
			+ "and (ISNUMERIC(t1.MTH) = 1 and t1.MTH <= '12') "
			//+ "and ISNUMERIC(DDAY) = 1 "
			+" and (dday <= '31')"
			+ "and ("
			+ "	('' = ?8 or CONVERT(datetime, (case when DDAY is null or dday = '' then '01' else DDAY end)+'/'+t1.MTH+'/'+t1.YEAR,103) >= CONVERT(datetime, ?8, 103)) "
			+ "	and "
			+ "	('' = ?9 or CONVERT(datetime, (case when DDAY is null or dday = '' then '01' else DDAY end)+'/'+t1.MTH+'/'+t1.YEAR,103) <= CONVERT(datetime, ?9, 103)) "
			+ "	) "
			+ ") tbl "
			, nativeQuery = true)
