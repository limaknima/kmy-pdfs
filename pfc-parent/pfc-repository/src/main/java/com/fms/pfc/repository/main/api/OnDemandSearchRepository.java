...
public interface OnDemandSearchRepository extends JpaRepository<OnDemandSearch, String> {
	@Query(value = "select * from ( "
			...
			//+ "and t1.DDAY < 32 "
			//+ "and DDAY !='' "
			+ "and ISNUMERIC(t1.YEAR) = 1 "
			+ "and ISNUMERIC(t1.MTH) = 1 "
			//+ "and ISNUMERIC(DDAY) = 1 "
			+ "and ('' = ?8 or CONVERT(datetime, (case when DDAY is null or dday = '' then '01' else DDAY end)+'/'+t1.MTH+'/'+t1.YEAR,103) >= CONVERT(datetime, ?8, 103)) "
			+ "and ('' = ?9 or CONVERT(datetime, (case when DDAY is null or dday = '' then '01' else DDAY end)+'/'+t1.MTH+'/'+t1.YEAR,103) <= CONVERT(datetime, ?9, 103)) "
			+ ") tbl "
			, nativeQuery = true)
