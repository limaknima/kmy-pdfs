...
public interface HplSearchRepository extends JpaRepository<HplSearch, Integer> {

	@Query(value = "select * from ( "
			+ "select "
			+ "	h.*, "
			+ "	(STUFF( "
			+ "			( SELECT top 10 ',' + CONVERT(NVARCHAR(max), rm.NAME ) "
			...
			+ ") "
			, nativeQuery = true)
	List<HplSearch> searchByCriteria(String hplName, String hplNameExp, String hplModelName, String hplModelNameExp);
}
